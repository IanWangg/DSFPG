...
class DSFPGAgent_td3(BaseAgent):
    ...
    def step_online(self):
        config = self.config
        # under offline setting, there is no need to collect trajectories
        # neither need to gather online return
        ...
        if self.total_steps >= config.warm_up or self.offline:
            ...
            # backprop
            # for critic
            # try directly using target_network for 
            phi_dot_w = self.network.critic(phi_next)
            critic_loss = F.mse_loss(phi_dot_w, rewards)
