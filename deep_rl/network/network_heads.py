...
class DSFPG_TD3Net(nn.Module, BaseNet):
    def __init__(self,
                 state_dim,
                 action_dim,
                 actor_body_fn,
        ...
        super(TD3Net, self).__init__()
        # there is no phi body in td3, the raw observation is the phi in td3
        self.actor_body = actor_body_fn()
        self.sf_body_1 = sf_body_fn()
        self.sf_body_2 = sf_body_fn()

        self.fc_action = layer_init(nn.Linear(self.actor_body.feature_dim, action_dim), 1e-3)
        # sf body output 
        self.fc_sf_1 = layer_init(nn.Linear(self.sf_body_1.feature_dim + action_dim, state_dim), 1e-3)
        self.fc_sf_2 = layer_init(nn.Linear(self.sf_body_2.feature_dim + action_dim, state_dim), 1e-3)

        self.actor_params = list(self.actor_body.parameters()) + list(self.fc_action.parameters())
        self.sf_params = list(self.fc_sf_1.parameters()) + list(self.fc_critic_1.parameters()) +\
                             list(self.fc_sf_2.parameters()) + list(self.fc_critic_2.parameters())
